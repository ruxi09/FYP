...
public:
    ...
    int operator()(int x, int y, bool condition) {
        ...
        return boost::contract::call_if<std::integral_constant<bool, true>>(
            [&](){
                // This lambda function will be called if condition is true.
                return m_func1(x, y);
            }).else_(
            [&](){
                // This lambda function will be called if condition is false.
                return m_func2(x, y);
            });
    }
